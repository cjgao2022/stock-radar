...
# ── 申万二级行业 ──────────────────────────────────────────────────
_l2_map_cache: tuple | None = None
_L2_MAP_TTL = 86400  # 24h


def _build_l2_industry_map() -> dict:
    """
    返回 {ind_code: {'name': str, 'stocks': [6位代码, ...]}}
    调用 sw_index_second_info + index_stock_cons × ~100，约 100s。
    """
    with _AK_LOCK:
...
def _get_l2_industry_map() -> dict:
    global _l2_map_cache
    now = time.time()
    if _l2_map_cache and now - _l2_map_cache[1] < _L2_MAP_TTL:
        return _l2_map_cache[0]
    data = _build_l2_industry_map()
    _l2_map_cache = (data, now)
    return data


def fetch_leaders_second() -> list[dict]:
    """
    每个申万二级行业 TOP5 龙头（按今日成交额降序）。

    耗时说明：
      - 首次（当天第一次调用）：_build_l2_industry_map 约 100s + stock_zh_a_spot 约 33s ≈ 2-3 分钟
      - 之后：行业映射命中 24h 缓存，stock_zh_a_spot 约 33s
      - 路由层再缓存整体结果 30 分钟
    """
    industry_map = _get_l2_industry_map()

    # 二级行业实时涨跌幅（指数级别）
    try:
        with _AK_LOCK:
            rt_df = ak.index_realtime_sw(symbol='二级行业')
        ind_rt: dict[str, dict] = {}
        for _, row in rt_df.iterrows():
            code = str(row['指数代码'])
            prev = float(row.get('昨收盘', 0) or 0)
